...
FORMAT_LINE_RANGES = {1: (10311, 10328), 2: (3046, 3065)}
...
class _LineCursor:
    ...
    def skip(self, count: int) -> None:
        if count < 0:
            raise ValueError(
                f"_LineCursor.skip does not support negative count: {count}"
            )
...
def _detect_format(lines: list[str]) -> int:
    count = len(lines)
    for save_format, (minimum, maximum) in FORMAT_LINE_RANGES.items():
...
def parse_save_lines(lines: list[str], chapter: int, slot: int) -> dict:
    ...
    for _ in range(character_count):
        character = {
            ...
            "weaponStyle": cursor.next_string()
            if save_format == 1
            else int(cursor.next_number()),
            "weaponStats": [],
            "spells": [],
        }
        for _ in range(4):
    ...
    flags_count = len(lines) - cursor.index - 3
    minimum_flags = 9999 if save_format == 1 else 2500
    if flags_count < minimum_flags:
        raise ValueError(
            f"Invalid save format ({flags_count} flags, expected at least {minimum_flags})"
        )
...
def serialize_save_data(save: dict) -> list[str]:
    ...
    lines.extend(
        _serialize_number(save[key]) for key in ("money", "xp", "lv", "inv", "invc")
    )
    ...
    flags_count = len(save["flags"])
    for idx in range(flags_count):
        lines.append(
            _serialize_number(save["flags"][idx] if idx < len(save["flags"]) else 0)
        )
...
def load_simple_mode_data() -> dict:
    try:
        with open(
            os.path.join(os.path.dirname(__file__), "simple_mode_data.json"),
            encoding="utf-8",
        ) as handle:
...
class SaveEditorDialog(QDialog):
    def __init__(
        ...
        btn_bar.addWidget(self.redo_btn)
        btn_bar.addStretch()
        ...
        self._history_past: list[list[str]] = []
        self._history_future: list[list[str]] = []
        self._history_snapshot = list(self._original_lines)
    ...
    def relocalize_ui(self) -> None:
        ...
        self.undo_btn.setText(self.tr_func("ui.undo"))
        self.redo_btn.setText(self.tr_func("ui.redo"))
    ...
    def _load_file(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "SaveEditorDialog: file contains invalid UTF-8 bytes: %s",
                self.file_path,
            )
            QMessageBox.warning(
                self,
                self.tr_func("errors.error"),
                self.tr_func(
                    "dialogs.save_file_error",
                    error=self.tr_func("dialogs.invalid_utf8"),
                ),
            )
            ...
            QMessageBox.warning(
                self,
                self.tr_func("errors.error"),
                self.tr_func("dialogs.save_file_error", error=str(error)),
            )
    ...
    def _set_advanced_lines(self, lines: list[str]) -> None:
        ...
        for row, line in enumerate(lines):
            ...
            number_item = QTableWidgetItem(
                f"{row + 1}{f' ({label})' if self.advanced_details_toggle.isChecked() and label else ''}"
            )
            field_item = QTableWidgetItem(
                details if self.advanced_details_toggle.isChecked() else label
            )
    ...
    def _try_build_simple_mode(self) -> None:
        try:
            ...
            self.save_data = parse_save_lines(
                self._current_advanced_lines(), chapter, slot
            )
    ...
    def _format_option_label(self, group: str, item_id: int) -> tuple[str, str]:
        ...
        return f"{label} ({item_id})", "\n\n".join(
            part for part in tooltip_parts if part
        )
    ...
    def _make_combo(
        self, current_id: int, options: list[tuple], on_change
    ) -> QComboBox:
        combo = QComboBox()
        ...
        if current_id not in seen:
            combo.addItem(
                f"{current_id} - {self.tr_func('ui.unknown_value')}", current_id
            )
            combo.setItemData(
                combo.count() - 1, str(current_id), Qt.ItemDataRole.ToolTipRole
            )
    ...
    def _option_list(
        self,
        group: str,
        allowed: list[int] | set[int],
        include_current: int | None = None,
        ...
        return sorted(
            (
                (int(item_id), *self._format_option_label(group, int(item_id)))
    ...
    def _chapter_content(self) -> dict:
        return self.simple_mode_data["chapters"]["meta"][
            str(self.save_data["meta"]["chapter"])
        ]["content"]
    ...
    def _advanced_labels(self, lines: list[str]) -> list[tuple[str, str]]:
        ...
        add_indexed(
            "unusedHeader",
            5,
            lambda idx: self.tr_func("ui.reserved_line_label", number=idx + 1),
        )
        add_indexed(
            "party", 3, lambda idx: f"{self.tr_func('ui.party_member')} {idx + 1}"
        )
        for key, title in (
            ("money", self.tr_func("ui.money_label")),
            ("xp", self.tr_func("ui.xp")),
            ("lv", self.tr_func("ui.level")),
            ("inv", self.tr_func("ui.inventory_count")),
            ("invc", self.tr_func("ui.inventory_capacity")),
            ("inDarkWorld", self.tr_func("ui.in_dark_world")),
        ):
            add(key, title)
        for character_id in range(4 if save_format == 1 else 5):
            character_name = _safe_name(
                characters_meta.get(str(character_id)), f"Character {character_id + 1}"
            )
            prefix = f"characters[{character_id}]"
            for key, title in (
                ("health", self.tr_func("ui.current_hp")),
                ("maxHealth", self.tr_func("ui.max_hp")),
                ("attack", self.tr_func("ui.attack")),
                ("defence", self.tr_func("ui.defence")),
                ("magic", self.tr_func("ui.magic")),
                ("guts", self.tr_func("ui.guts")),
                ("weapon", self.tr_func("ui.weapon_label")),
                ("primaryArmor", self.tr_func("ui.armor_one")),
                ("secondaryArmor", self.tr_func("ui.armor_two")),
                ("weaponStyle", self.tr_func("ui.weapon_style")),
            ):
            ...
            for weapon_idx in range(4):
                ...
                for key in (
                    ...
                    add(
                        f"{weapon_prefix}.{key}",
                        f"{character_name} {self.tr_func('ui.weapon_set_label', number=weapon_idx + 1)}: {key}",
                    )
                if save_format == 2:
                    add(
                        f"{weapon_prefix}.element",
                        f"{character_name} {self.tr_func('ui.weapon_set_label', number=weapon_idx + 1)}: {self.tr_func('ui.element')}",
                    )
                    add(
                        f"{weapon_prefix}.elementAmount",
                        f"{character_name} {self.tr_func('ui.weapon_set_label', number=weapon_idx + 1)}: {self.tr_func('ui.element_power')}",
                    )
            add_indexed(
                f"{prefix}.spells",
                12,
                lambda idx, name=character_name: (
                    f"{name}: {self.tr_func('ui.spell_label')} {idx + 1}"
                ),
            )
        for key in ("boltSpeed", "grazeAmount", "grazeSize"):
        ...
        add(
            "battle.tension",
            f"{self.tr_func('ui.battle')}: {self.tr_func('ui.current_tp')}",
        )
        add(
            "battle.maxTension",
            f"{self.tr_func('ui.battle')}: {self.tr_func('ui.max_tp')}",
        )
        for key, title in (
            ("weapon", self.tr_func("ui.weapon_label")),
            ("armor", self.tr_func("ui.armor_label")),
            ("experience", self.tr_func("ui.experience")),
            ("level", self.tr_func("ui.level")),
            ("money", self.tr_func("ui.money_label")),
            ("health", self.tr_func("ui.current_hp")),
            ("maxHealth", self.tr_func("ui.max_hp")),
            ("attack", self.tr_func("ui.attack")),
            ("defence", self.tr_func("ui.defence")),
            ("weaponStrength", self.tr_func("ui.weapon_strength")),
            ("armorDefence", self.tr_func("ui.armor_defence")),
        ):
            add(f"lightWorld.{key}", f"{self.tr_func('ui.light_world')}: {title}")
        add_indexed(
            "lightWorld.items",
            8,
            lambda idx: f"{self.tr_func('ui.items_label')} {idx + 1}",
        )
        add_indexed(
            "lightWorld.phone",
            8,
            lambda idx: f"{self.tr_func('ui.phone_contacts')} {idx + 1}",
        )
        flags_count = max(0, len(lines) - len(rows) - 3)
        ...
        for flag_id in range(flags_count):
            ...
            add(
                f"flags[{flag_id}]",
                f"{_safe_name(meta, name)}: {meta.get('description', name)}".strip(
                    ": "
                ),
            )
        add("plot", self.tr_func("ui.plot_label"))
        room_name = _safe_name(
            rooms_meta.get(str(_parse_number(lines[-2]) if len(lines) > 1 else 0)),
    ...
    def _flag_editor(self, flag_id: int) -> QWidget:
        ...
        inferred_boolean = (
            value_type == "number"
            and not rules.get("map")
            and int(rules.get("min", -999999)) == 0
            and int(rules.get("max", 999999)) == 1
        )
        if value_type == "boolean" or inferred_boolean:
            ...
            editor.stateChanged.connect(
                lambda state, fid=flag_id, inv=inverted: self._set_flag(
                    fid, 0 if state and inv else 1 if state else 1 if inv else 0
                )
            )
        elif value_type == "map" and rules.get("map"):
            items = sorted(
                ((_parse_number(key), label) for key, label in rules["map"].items()),
                key=lambda item: float(item[0]),
            )
            editor = self._make_combo(
                value,
                items,
                lambda selected, fid=flag_id: self._set_flag(
                    fid, _parse_number(str(selected or 0))
                ),
            )
        elif value_type == "color":
            editor = self._make_spin(
                value,
                lambda new_value, fid=flag_id: self._set_flag(fid, new_value),
                0,
                31,
            )
        else:
            editor = self._make_spin(
                value,
                lambda new_value, fid=flag_id: self._set_flag(fid, new_value),
                int(rules.get("min", -999999)),
                int(rules.get("max", 999999)),
            )
    ...
    def _build_customization_page(self) -> QWidget:
        ...
        for title_key, names in CUSTOMIZATION_FLAG_GROUPS:
            ...
            if visible_names:
                layout.addWidget(
                    self._flag_group_widget(self.tr_func(title_key), visible_names)
                )
        layout.addStretch()
        return page
    ...
    def _build_battle_page(self) -> QWidget:
        ...
        for key, title, minimum, maximum in (
            ...
            battle_layout.addRow(
                title,
                self._make_spin(
                    battle[key],
                    lambda value, field=key: battle.__setitem__(field, value),
                    minimum,
                    maximum,
                ),
            )
    ...
    def _build_general_page(self) -> QWidget:
        ...
        plot_data = self.simple_mode_data.get("plotPoints", {}).get(
            str(self.save_data["meta"]["chapter"]), {}
        )
        plot_options = [
            (
                int(value),
                _safe_name(plot_data.get("meta", {}).get(str(value)), name),
                str(
                    plot_data.get("meta", {}).get(str(value), {}).get("description", "")
                ),
            )
            for name, value in plot_data.get("ids", {}).items()
        ]
        if not any(
            value == int(self.save_data["plot"]) for value, _label, _tip in plot_options
        ):
            plot_options.append(
        ...
        info_layout.addRow(
            ...
            self._make_combo(
                ...
                plot_options,
                lambda value: self.save_data.__setitem__("plot", int(value or 0)),
            ),
        )
        info_layout.addRow(
            self.tr_func("ui.room_label"),
            self._make_combo(
                self.save_data["room"],
                self._option_list(
                    "rooms", self._chapter_content()["rooms"], self.save_data["room"]
                ),
    ...
    def _story_field_editor(self, name: str) -> QWidget | None:
        flag_id = self._flag_id(name)
        if flag_id is not None:
            return self._flag_editor(flag_id)
        meta = self.simple_mode_data.get("flagBitfields", {}).get("meta", {}).get(name)
        if not isinstance(meta, dict):
            return None
        parent = int(meta.get("parent", -1))
        index = int(meta.get("index", 0))
        width = max(1, int(meta.get("width", 1)))
        if not 0 <= parent < len(self.save_data["flags"]):
            return None
        mask = (1 << width) - 1
        current = (int(self.save_data["flags"][parent]) >> index) & mask

        def update(value: int) -> None:
            parent_value = int(self.save_data["flags"][parent])
            self._set_flag(
                parent, (parent_value & ~(mask << index)) | ((value & mask) << index)
            )

        row = QWidget()
        row_layout = QHBoxLayout(row)
        row_layout.setContentsMargins(0, 0, 0, 0)
        label = QLabel(_safe_name(meta, name))
        if meta.get("description"):
            label.setToolTip(str(meta["description"]))
        row_layout.addWidget(label, 1)
        row_layout.addWidget(self._make_spin(current, update, 0, mask))
        return row
    ...
    def _build_party_overview_page(self) -> QWidget:
        ...
        for idx in range(3):
            layout.addRow(
                f"{self.tr_func('ui.party_member')} {idx + 1}",
                self._make_combo(
                    self.save_data["party"][idx],
                    options,
                    lambda value, index=idx: self.save_data["party"].__setitem__(
    ...
    def _build_character_page(self, character_id: int) -> QWidget:
        ...
        stats_box, stats_layout = self._section(
            _safe_name(character_meta, str(character_id))
        )
        for key, title in (
            ("health", self.tr_func("ui.current_hp")),
            ("maxHealth", self.tr_func("ui.max_hp")),
            ...
            stats_layout.addRow(
                ...
                self._make_spin(
                    character[key],
                    lambda value, field=key: character.__setitem__(field, value),
                    0,
                    99999,
                ),
            )
        allowed_weapons = (
            chapter["weapons"]
            if not character_meta.get("allowedWeapons")
            else [
                item
                for item in chapter["weapons"]
                if item in character_meta["allowedWeapons"]
            ]
        )
        allowed_armors = (
            chapter["armors"]
            if not character_meta.get("allowedArmors")
            else [
                item
                for item in chapter["armors"]
                if item in character_meta["allowedArmors"]
            ]
        )
        allowed_spells = (
            chapter["spells"]
            if not character_meta.get("allowedSpells")
            else [
                item
                for item in chapter["spells"]
                if item in character_meta["allowedSpells"]
            ]
        )
        stats_layout.addRow(
            ...
            self._make_combo(
                ...
                lambda value: character.__setitem__("weapon", int(value or 0)),
            ),
        )
        for key, title in (
            ("primaryArmor", self.tr_func("ui.armor_one")),
            ...
            stats_layout.addRow(
                ...
                self._make_combo(
                    character[key],
                    self._option_list("armors", allowed_armors, character[key]),
                    lambda value, field=key: character.__setitem__(
        ...
        for idx in range(len(character["spells"])):
            ...
            spells_layout.addWidget(
                QLabel(f"{self.tr_func('ui.spell_label')} {idx + 1}"), row, column
            )
            spells_layout.addWidget(
                self._make_combo(
                    character["spells"][idx],
                    self._option_list(
                        "spells", allowed_spells, character["spells"][idx]
                    ),
                    lambda value, index=idx: character["spells"].__setitem__(
                        index, int(value or 0)
                    ),
                ),
                row,
        ...
        if self.save_data["meta"]["format"] == 2:
            stat_titles = (
                *stat_titles,
                ("element", self.tr_func("ui.element")),
                ("elementAmount", self.tr_func("ui.element_power")),
            )
        for weapon_idx, weapon_stats in enumerate(character["weaponStats"]):
            ...
            for row, (key, label) in enumerate(stat_titles, start=1):
                ...
                weapon_layout.addWidget(
                    self._make_spin(
                        weapon_stats.get(key, 0),
                        lambda value, stats=weapon_stats, field=key: stats.__setitem__(
                            field, value
                        ),
                        -99999,
                        99999,
    ...
    def _build_inventory_page(self, key: str, group: str) -> QWidget:
        ...
        for idx, value in enumerate(values):
            layout.addWidget(
                QLabel(f"{self.tr_func('ui.slot_label')} {idx + 1}"), idx, 0
            )
            layout.addWidget(
                self._make_combo(
                    value,
                    self._option_list(group, chapter[allowed_key], value),
                    lambda selected, index=idx: values.__setitem__(
                        index, int(selected or 0)
                    ),
                ),
                idx,
                1,
    ...
    def _build_light_world_page(self) -> QWidget:
        ...
        for key, title in (
            ...
            stats_layout.addRow(
                ...
                self._make_spin(
                    light_world[key],
                    lambda value, field=key: light_world.__setitem__(field, value),
                    0,
                    99999,
                ),
            )
        ...
        weapon_options = [
            item_id
            for item_id in light_world_items
            if self._group_meta("lightWorldItems").get(str(item_id), {}).get("weapon")
        ]
        armor_options = [
            item_id
            for item_id in light_world_items
        ...
        stats_layout.addRow(
            ...
            self._make_combo(
                light_world["weapon"],
                self._option_list(
                    "lightWorldItems", weapon_options, light_world["weapon"]
                ),
                lambda value: light_world.__setitem__("weapon", int(value or 0)),
            ),
        )
        stats_layout.addRow(
            ...
            self._make_combo(
                light_world["armor"],
                self._option_list(
                    "lightWorldItems", armor_options, light_world["armor"]
                ),
                lambda value: light_world.__setitem__("armor", int(value or 0)),
            ),
        )
        layout.addWidget(stats_box)
        ...
        for idx in range(8):
            ...
            items_layout.addWidget(
                self._make_combo(
                    light_world["items"][idx],
                    self._option_list(
                        "lightWorldItems",
                        chapter["lightWorld"]["items"],
                        light_world["items"][idx],
                    ),
                    lambda value, index=idx: light_world["items"].__setitem__(
                        index, int(value or 0)
                    ),
                ),
                idx,
        ...
        for idx in range(8):
            ...
            phone_layout.addWidget(
                self._make_combo(
                    light_world["phone"][idx],
                    self._option_list(
                        "phoneContacts",
                        chapter["lightWorld"]["phoneContacts"],
                        light_world["phone"][idx],
                    ),
                    lambda value, index=idx: light_world["phone"].__setitem__(
                        index, int(value or 0)
                    ),
                ),
                idx,
    ...
    def _build_recruits_page(self) -> QWidget:
        ...
        for enemy_id in self._chapter_content()["enemies"]:
            ...
            current_count = (
                current_value
                if recruit_count <= 1 or current_value in (0, -1)
                else current_value * recruit_count
            )
            row = QWidget()
            row_layout = QHBoxLayout(row)
            row_layout.setContentsMargins(0, 0, 0, 0)
            ...
            if recruit_count <= 1:
                check.stateChanged.connect(
                    lambda state, fid=recruit_flag: self._set_flag(
                        fid, 1 if state else 0
                    )
                )
            else:
                count_spin = self._make_spin(
                    current_count,
                    lambda value, fid=recruit_flag, rc=recruit_count, box=check: (
                        set_recruit_value(fid, rc, value),
                        box.setChecked(value == rc),
                    ),
                    -1,
                    recruit_count,
                )
                check.stateChanged.connect(
                    lambda state, fid=recruit_flag, rc=recruit_count, spin=count_spin: (
                        spin.setValue(rc if state else 0)
                    )
                )
    ...
    def _build_flags_page(self) -> QWidget:
        ...
        def sync_manual_value() -> None:
            flag_value_input.setText(
                _serialize_number(self.save_data["flags"][flag_id_input.value()])
            )

        def apply_manual_value() -> None:
            self._set_flag(
                flag_id_input.value(), _parse_number(flag_value_input.text())
            )
        ...
        for flag_id in self._chapter_content()["flags"]:
            ...
            haystack = (
                self._flag_name(flag_id)
                + " "
                + self._flag_meta(flag_id).get("description", "")
            ).lower()
        ...
        search.textChanged.connect(
            lambda text: [
                row.setVisible(
                    text.lower().strip() in haystack if text.strip() else True
                )
    ...
    def _build_story_chapter_page(self, chapter: int) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        sections = self.simple_mode_data.get("storySections", {}).get(str(chapter), [])
        for section in sections:
            section_box, section_layout = self._section(
                str(section.get("title", "Story")), QVBoxLayout
            )
            clusters = section.get("clusters")
            groups = clusters if isinstance(clusters, list) else [section]
            for group in groups:
                title = group.get("title")
                if title and clusters:
                    section_layout.addWidget(QLabel(str(title)))
                for name in group.get("flags", []):
                    editor = self._story_field_editor(str(name))
    ...
    def _rebuild_simple_mode(self) -> None:
        ...
        for name in ("KRIS", "SUSIE", "RALSEI", "NOELLE"):
            ...
            if (
                ...
                or character_id >= len(self.save_data["characters"])
                or character_id not in self._chapter_content()["characters"]
            ):
                continue
            party_pages.append(
                (
                    _safe_name(
                        self._group_meta("characters").get(str(character_id)),
                        name.title(),
                    ),
                    lambda current_id=character_id: self._wrap_scroll(
                        self._build_character_page(current_id)
                    ),
                )
            )
        inventory_pages = [
        ...
        if index == 0:
            try:
                ...
                QMessageBox.warning(
                    self,
                    self.tr_func("errors.error"),
                    self.tr_func("dialogs.simple_mode_parse_error", error=str(error)),
                )
    ...
    def _current_lines_for_save(self) -> list[str]:
        return (
            serialize_save_data(self.save_data)
            if self.mode_tabs.currentIndex() == 0 and self._simple_ready
            else self._current_advanced_lines()
        )

    def _capture_history(self) -> None:
        if self._history_applying or self._load_failed:
            return
        current = self._current_lines_for_save()
        if current == self._history_snapshot:
            return
        self._history_past.append(self._history_snapshot)
        self._history_past = self._history_past[-50:]
        self._history_future.clear()
        self._history_snapshot = list(current)
        self._update_history_buttons()

    def _apply_history(self, lines: list[str]) -> None:
        self._history_applying = True
        try:
            chapter, slot = self._save_identity()
            self._advanced_lines_cache = list(lines)
            self.save_data = parse_save_lines(lines, chapter, slot)
            self._simple_ready = True
            if self.mode_tabs.currentIndex() == 0:
                self._rebuild_simple_mode()
            else:
                self._set_advanced_lines(lines)
            self._history_snapshot = list(lines)
        finally:
            self._history_applying = False
            self._update_history_buttons()
    ...
    def _undo(self) -> None:
        ...
        if not self._history_past:
            return
        self._history_future.append(self._history_snapshot)
        self._apply_history(self._history_past.pop())

    def _redo(self) -> None:
        if not self._history_future:
            return
        self._history_past.append(self._history_snapshot)
        self._apply_history(self._history_future.pop())

    def _update_history_buttons(self) -> None:
        if hasattr(self, "undo_btn"):
            self.undo_btn.setEnabled(bool(getattr(self, "_history_past", [])))
            self.redo_btn.setEnabled(bool(getattr(self, "_history_future", [])))

    def _on_cancel(self) -> None:
        if self._current_lines_for_save() != self._original_lines:
            reply = QMessageBox.question(
                self,
                self.tr_func("dialogs.cancel_changes"),
                self.tr_func("dialogs.changes_will_be_lost"),
            )
    ...
    def _on_save(self) -> None:
        if self._load_failed:
            QMessageBox.warning(
                ...
                self.tr_func(
                    "dialogs.save_file_error",
                    error=self.tr_func("dialogs.cannot_save_unloaded"),
                ),
            )
            return
        ...
        if lines != self._original_lines:
            changed_count = sum(
                1
                for index in range(max(len(lines), len(self._original_lines)))
                if (lines[index] if index < len(lines) else None)
                != (
                    self._original_lines[index]
                    if index < len(self._original_lines)
                    else None
                )
            )
            reply = QMessageBox.question(
                self,
                self.tr_func("dialogs.save_changes"),
                self.tr_func("dialogs.changes_summary", count=changed_count),
            )
            if reply != QMessageBox.StandardButton.Yes:
                return
        tmp = None
        try:
            ...
            QMessageBox.critical(
                self,
                self.tr_func("dialogs.access_error"),
                self.tr_func(
                    "dialogs.no_write_permissions", path=os.path.dirname(self.file_path)
                ),
            )
        except OSError as error:
            QMessageBox.critical(
            ...
            if tmp and os.path.exists(tmp):
                try:
                    ...
                    logger.warning(
                        "SaveEditorDialog: failed to remove temporary file: %s", tmp
                    )
